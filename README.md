...
Paste one URL per line. YouTube playlists are expanded into individual videos before download. The app accepts up to 600 videos in one run. Pressing Enter inside URLS adds a new line; only the GRAB button starts acquisition.

Large playlist runs are resumable: if you run the same playlist again into the same output folder, existing `source_url` values in `.source.json` files are skipped instead of downloaded twice.
...
This is a tiny mini player: logo folder pick, automatic BPM analysis, sorted queue, icon controls for play and next, and adaptive crossfade. Mixes stay short by default, then get longer when the current outro and next intro have usable music beds and the BPMs are close. Quit the app to stop playback. The only visible text in the player is the current track title. It does not tag, rewrite, normalize, beatmatch, or alter the source files.
